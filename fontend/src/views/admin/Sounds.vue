<template>
  ...
  :loading="loading" loading-text="Loading... Please wait">
    <template v-slot:top>
      ...
      <v-toolbar flat>
        <v-toolbar-title>Músicas</v-toolbar-title>
        <v-divider class="mx-4" inset vertical /> 
        <v-spacer />

        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on" >Nova Música</v-btn>
          </template>
          <v-card>
              ...
              <v-container>
                <v-form v-bind:disabled="loading" lazy-validation ref="dialogForm">
                  ...
                  <v-col cols="12" sm="6" md="6">
                    <v-text-field :disabled="loading" :rules="titleRule" v-model="editedItem.title" label="Nome da Música"></v-text-field>
                  </v-col>
                  <v-col cols="12" sm="6" md="6">
                    <input type="file" class="d-none" ref="uploader" accept=".mp3" @change="selectFile()">
                    <v-text-field :rules="soundRule" :disabled="loading" v-model="editedItem.sound" label="Url" prepend-icon="mdi-music-box" @click:prepend="$refs.uploader.click()">
                    </v-text-field>
                  </v-col>

                  <v-col cols="12" sm="12" md="12">                    
                    <v-select :items="categories" :rules="categoryRule" v-model="editedItem.category_id" name="category" 
                    item-text="title" item-value="id" label="Selecione uma Categoria" solo ></v-select>
                  </v-col>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn :disabled="loading" color="blue darken-1" text @click="'1'">Cancelar</v-btn>
              <v-btn :disabled="loading" color="blue darken-1" text @click="save">Salvar</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
...
export default {
  data() {
    return {
      ...
      soundRule: [
        v => /^(?:(?:https?|ftp):\/\/)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:\/\S*)?$/.test(v) || 'deve-se ser uma url válida',
      ],
      categoryRule: [
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1 ? "Nova Música" : "Editar Música";
    },
  },
    ...
    async deleteItem(item) {
      ...
      if (confirm("Are you sure you want to delete this item?")) {
        ...
        try {
          ...
          this.snackText = 'Música foi deletada';
